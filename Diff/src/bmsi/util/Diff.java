...
public class Diff {
	...
	class FileData {
		...
		FileData(Object[] data, HashMap<Object, Integer> h) {
	...
	public interface ScriptBuilder {
		 ...
		 */
		public Change buildScript(boolean[] changed0, int len0,
	...
	public Diff(Object[] a, Object[] b) {
		HashMap<Object, Integer> h = new HashMap<Object, Integer>(a.length
				+ b.length);
			...
			if (c > 200 && big_snake && heuristic) {
				...
				for (d = fmax; d >= fmin; d -= 2) {
					...
					if (v > 12 * (c + (dd < 0 ? -dd : dd))) {
						if (v > best && xoff + Diff.SNAKE_LIMIT <= x
								&& x < xlim && yoff + Diff.SNAKE_LIMIT <= y
								&& y < ylim) {
							/*
				...
				for (d = bmax; d >= bmin; d -= 2) {
					...
					if (v > 12 * (c + (dd < 0 ? -dd : dd))) {
						if (v > best && xoff < x
								&& x <= xlim - Diff.SNAKE_LIMIT && yoff < y
								&& y <= ylim - Diff.SNAKE_LIMIT) {
							/*
							 * We have a good enough best diagonal; now insist
							 * that it end with a significant snake.
							 */
	 ...
	 */
	private Change diff(final ScriptBuilder bld) {
		...
		int diags = filevec[0].nondiscardedLines + filevec[1].nondiscardedLines
				+ 3;
		fdiag = new int[diags];
		...
		compareseq(0, filevec[0].nondiscardedLines, 0,
				filevec[1].nondiscardedLines);
		fdiag = null;
		bdiag = null;
		 ...
		 */
		return bld.buildScript(filevec[0].changedFlag,
				filevec[0].bufferedLines, filevec[1].changedFlag,
				filevec[1].bufferedLines);

	}

	public final Change diff() {
		return diff(forwardScript);
	}

	public final Change diffReverse() {
		return diff(reverseScript);
	}
